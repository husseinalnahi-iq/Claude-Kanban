...
| D187 | `AskUserQuestion` is **allowed in both modes** and becomes a question card on the approvals machinery (same resolver, status `approval`, alert and sound); the answer returns as the tool's own `updatedInput.answers` (question text → label, several comma-separated); **Let Claude decide** is a deny telling it to choose and say which. Default **waits forever**; `questionWaitMin` > 0 decides after that many minutes. Supersedes the `AskUserQuestion` part of D19 | A guess at the one decision only the user can make costs more than a wait. The runner handles the tool before the autonomous gate, so the gate's refusal stays only as a fallback. Waiting forever is the user's chosen default; the timer is the escape hatch for night runs |
| D188 | The **side chat reads and makes cards but never edits**: read tools only, `Edit`/`Write`/`Bash`/subagents disallowed, its own board server limited to its project; one resumable session per chat, project `CLAUDE.md` only (no global MCP servers); default **Sonnet 5 / medium** | A chat that could edit would be a second, unsupervised path around worktrees, approvals and review. Cards keep every change on the board's rails. Sonnet at medium answers code questions well at a fraction of Opus's price; each chat can switch |
| D189 | The **built-in terminal** runs your shell on the server via `node-pty` as an **optionalDependency**, with a pipe-based **basic mode** fallback; shells open only in a registered project or its task worktrees, over a guarded `/ws/terminal/:id` socket kept off the Bus; scrollback is replayed on reopen; all shells end on server close | Non-technical users cannot fix a failed native build, so the install must never depend on it. The terminal is the user's own, so the board's command guardrails do not apply, but where it can open and who can reach it do |
| D190 | The **live browser view** reuses the task's own Playwright browser: its launch config adds a `--remote-debugging-port`, and the board screencasts over CDP **only while someone watches** (≈5 fps, frames acked, page brought to front with focus emulation); the view follows the last-navigated site, else a local page, never an extension tab; the last frame outlives the run | A second browser or a visible window would double the cost or cover the user's work. Streaming only to viewers keeps idle runs free. A company-installed extension opened a welcome tab in front of the task's page in testing, which both stole the view and starved its frames |
| D191 | The terminal's PowerShell starts with **`-ExecutionPolicy RemoteSigned` for that process only**, and Windows PowerShell with UTF-8 console encoding; Setup offers **Repair** (`npm install` in the board folder, for the optional `node-pty`) and an optional **PowerShell 7** install; a failed `node-pty` load is not cached, so a repair takes effect without a restart | On a fresh Windows `npm`/`npx` are `.ps1` scripts that the default policy refuses, which would make the first command a user types fail. A process-scoped policy fixes that without touching the machine's setting |
| D192 | The **Claude model list** comes from Claude Code itself (`supportedModels()` in the SDK's startup handshake, prompt never sent, session closed; cached 30 min, a failure 30 s) and every Claude pick is checked against it: **invalid** (not shaped like a Claude id) is red and fails a run, **unlisted** is amber and only a likely typo, since Claude Code still accepts older ids it no longer lists; picks show a ⚠, Settings a banner, Setup a row; the list's ids are read-only, new ones come from a dropdown | The list was free text, so a typo (`test`, `claude-sonet-5`) surfaced only as a failed run. The API's model list needs an API key that subscription users do not have; Claude Code's own picker list is what the login can use, and free to read |
| D193 | **✦ Rewrite** on a task's Spec is a separate path from Improve: one read-only session (`Read`/`Glob`/`Grep`, project CLAUDE.md, `dontAsk`, structured output, $3 ceiling) on **Opus 5 at high by default** (Settings → Spec rewrite; per rewrite any Claude model and effort, plus an optional focus). Every version is kept in `spec_versions`; a rewrite always starts from **your latest words**, never an earlier rewrite; restoring first saves the current text. It is refused while the task is queued or running | Improve is the cheap, code-blind intake call that also sizes and splits; a spec worth a strong model needs the code, and the cost of one Opus call is small next to a run that goes the wrong way. Rewriting a rewrite drifts from what the person asked, and comparing models is only fair on the same input |

## Subscriptions for other models, running out, and their usage (2026-09-12)

Requested: "plan for case like the delegated ai don't have credit or it end during execution, what we should do? Ask me to switch to another provider or ask me to hold this task till go back and resume", "How we can see other models usage?", with GLM, Kimi and Qwen subscriptions (not pay-per-use) in mind.

| # | Decision | Why |
|---|---|---|
| D194 | A delegated stage's failure is classified as **window** (comes back by itself), **credit** (does not) or **busy**, else it is an ordinary failure. The provider is recorded as out (`provider_limits`) with a reset time from the message, the provider's usage API, or a 30-minute retry that doubles to 4 hours. Then: its **fallback** (`Provider.fallback`, off by default) takes the stage over at once; else a window **pauses** (`pause_reason: "provider"`, with `resume_at`) and resumes in the same session; credit pauses with **no** `resume_at` and is reported as *needs you*, offering **Switch & continue**, **Try again** and **Stop**. A switch re-runs the stage on the new model with a **handover** in its prompt (where the changes are, what the last model said); at most three moves per run. Queued work for a provider out until a known time waits in the queue; a stage about to start on it pauses without calling it. `Settings.claudeFallback` does the same for Claude's own limit. Replaces the old rule that a foreign provider's rate limit just fails the task | The answer to "switch or wait?" depends on which kind of out it is: a 5-hour window is back before morning, so waiting is right and costs nothing; empty credit never comes back, so only a person can decide, and failing the task would throw its progress away. A fallback is the user saying in advance which they prefer. Resuming a session on a different model risks thinking blocks the new endpoint rejects, so a switch starts a fresh session and hands over in words instead, with the files already in place. Holding the queue stops ten tasks each failing on the same empty plan |
| D195 | **Provider usage** is read from the provider where it offers a read-only endpoint (z.ai's quota page API, Kimi Code's `/coding/v1/usages`, OpenRouter's credits, Moonshot's balance), cached 5 minutes, key sent only to that provider's host; for everyone else the board shows what **its own runs** sent there (5 hours, 7 days). A window the provider reports as used up marks it out before a stage fails on it; room again clears it. Shown under Claude's windows in the usage panel, on each provider in Settings, and as a top-bar chip when one is out | A subscription's limit is set by the provider, not by the board's token count, so the provider's own figure is the one worth showing when there is one. Not every provider has such an endpoint (Ollama, Alibaba, MiniMax), and a board-side count is honest about being only the board's share |
| D196 | **Kimi Code** is its own preset (`api.kimi.com/coding/`, key sent as `ANTHROPIC_API_KEY` via `authStyle: "api-key"`), separate from the pay-per-use Kimi API; an **Alibaba Token Plan** preset (Singapore) warns that Alibaba permits interactive use only; the Ollama preset says how to use its cloud models (Ollama Pro) but lists none, since the picker shows Ollama's cloud list live and a listed model would become a Setup "pull" row. `ANTHROPIC_DEFAULT_FABLE_MODEL` is pinned with the other aliases | The old Kimi preset pointed at Moonshot's pay-per-token address, where a Kimi Code subscription key does not work. Alibaba's terms are the kind of thing a person should see before running it unattended all night |
